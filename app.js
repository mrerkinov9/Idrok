...
async function apiRequest(path, options={}){let response;try{response=await fetch(path,{...options,headers:{'Content-Type':'application/json',...(authToken?{Authorization:`Bearer ${authToken}`}:{}) ,...(options.headers||{})}})}catch{throw new Error(location.protocol==='file:'?'Idrok serveri ishga tushmagan. IDROKNI-OCHISH.cmd faylini oching.':'Serverga ulanib bo‘lmadi. IDROKNI-OCHISH.cmd orqali saytni qayta oching.')}const contentType=response.headers.get('content-type')||'',payload=contentType.includes('application/json')?await response.json().catch(()=>({})):{};if(!response.ok)throw new Error(payload.error||(!contentType.includes('application/json')?'Idrok noto‘g‘ri manzildan ochilgan. IDROKNI-OCHISH.cmd faylini oching.':'Server bilan aloqa xatosi.'));return payload}
...
document.querySelector('#registerForm').addEventListener('submit',async e=>{
  e.preventDefault();const form=e.currentTarget,data=new FormData(form),message=form.querySelector('.form-message'),submit=form.querySelector('[type="submit"]');
  message.textContent='';message.classList.remove('success');submit.disabled=true;submit.querySelector('span').textContent='Hisob yaratilmoqda…';
  try{rememberGuestSession();const result=await apiRequest('/api/register',{method:'POST',body:JSON.stringify({name:data.get('name').trim(),email:data.get('email').trim().toLowerCase(),password:data.get('password')})});authToken=result.token;localStorage.setItem('idrokAuthToken',authToken);applyAuthenticatedUser(result.user);saveState();form.reset();message.classList.add('success');sessionStorage.setItem('idrokAuthPreviewed','1');closeAll();showToast('Hisob yaratildi: +50 Impulse!');continueAfterAuth()}catch(error){message.textContent=error.message}finally{submit.disabled=false;submit.querySelector('span').textContent='Hisob yaratish'}
});
document.querySelector('#loginForm').addEventListener('submit',async e=>{
  e.preventDefault();const form=e.currentTarget,data=new FormData(form),message=form.querySelector('.form-message'),submit=form.querySelector('[type="submit"]');message.textContent='';message.classList.remove('success');submit.disabled=true;submit.querySelector('span').textContent='Tekshirilmoqda…';
  try{rememberGuestSession();const result=await apiRequest('/api/login',{method:'POST',body:JSON.stringify({email:data.get('email').trim().toLowerCase(),password:data.get('password')})});authToken=result.token;localStorage.setItem('idrokAuthToken',authToken);const user=applyAuthenticatedUser(result.user);form.reset();sessionStorage.setItem('idrokAuthPreviewed','1');closeAll();showToast(`Xush kelibsiz, ${user.name}!`);continueAfterAuth()}catch(error){message.textContent=error.message}finally{submit.disabled=false;submit.querySelector('span').textContent='Hisobga kirish'}
});
document.querySelectorAll('[data-password]').forEach(button=>button.addEventListener('click',()=>{const input=button.parentElement.querySelector('input');const showing=input.type==='text';input.type=showing?'password':'text';button.textContent=showing?'Ko‘rish':'Yashirish';button.setAttribute('aria-label',showing?'Parolni ko‘rsatish':'Parolni yashirish')}));
document.querySelector('#registerForm input[name="password"]').addEventListener('input',event=>{const value=event.target.value,meter=document.querySelector('.password-meter');let strength=0;if(value.length>=6)strength++;if(/[A-ZА-Я]/.test(value)&&/[a-zа-я]/.test(value))strength++;if(/\d/.test(value))strength++;if(/[^\w\s]/.test(value)&&value.length>=8)strength++;meter.dataset.strength=String(strength);meter.querySelector('small').textContent=['Parol kuchi','Kuchsiz','O‘rtacha','Yaxshi','Kuchli'][strength]});
...
renderCourses();updateDashboard();renderLeaderboard();bootstrapAccount();
