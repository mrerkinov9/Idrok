...
function continueAfterAuth(){
  ...
  if(next&&/^[a-z0-9_-]+\.html(?:[?#].*)?$/i.test(next)){location.href=next;return true}
...
function updateDashboard(){
  ...
  const user=currentUser(),name=user?.name||'Mehmon';document.querySelector('#accountName').textContent=name;document.querySelector('#accountAvatar').textContent=name.slice(0,2).toUpperCase();document.querySelector('#accountStatus').textContent=user?(adminMode?'Administrator':'Shaxsiy kabinet'):'Kirish yoki ro‘yxatdan o‘tish';document.querySelector('#headerAuthButton').textContent=user?'Kabinet':'Kirish';document.querySelector('#headerImpulse').querySelector('b').textContent=adminMode?'∞':state.impulse;const adminLink=document.querySelector('#adminPanelLink');if(adminLink)adminLink.hidden=!adminMode;
...
function openAuth(tab='login'){
  document.querySelector('#otpForm').classList.add('hidden');document.querySelector('.auth-panel').classList.remove('otp-active');
  ...
  setTimeout(()=>document.querySelector(`#${tab==='register'?'register':'login'}Form input`)?.focus(),180);
}
...
document.querySelectorAll('[data-open-auth]').forEach(button=>button.addEventListener('click',()=>openAuth(button.dataset.openAuth||'register')));
...
document.querySelector('#logoutButton').addEventListener('click',async()=>{
  ...
  document.body.classList.toggle('dark',state.theme==='dark');closeAll();updateDashboard();renderCourses();renderLeaderboard();showToast('Hisobdan chiqdingiz.');openAuth('login');
});
let pendingAuth=null;try{pendingAuth=JSON.parse(sessionStorage.getItem('idrokPendingAuth')||'null')}catch{pendingAuth=null}
function friendlyAuthError(error){const message=String(error?.message||'').toLowerCase();if(message.includes('email address not authorized'))return'Bu emailga xat yuborish uchun production email xizmati hali yoqilmagan.';if(message.includes('rate')||message.includes('seconds'))return'Juda ko‘p urinish bo‘ldi. Bir daqiqa kutib, qayta urinib ko‘ring.';if(message.includes('expired')||message.includes('invalid token')||message.includes('token has expired'))return'Kod eskirgan yoki noto‘g‘ri. Yangi kod so‘rang.';if(message.includes('provider is not enabled'))return'Google orqali kirish hali yoqilmagan. Administratorga xabar bering.';if(message.includes('signup')&&message.includes('disabled'))return'Yangi hisob yaratish vaqtincha yopiq.';return error?.message||'Kirishda kutilmagan xato yuz berdi.'}
...
function showOtp(pending){pendingAuth=pending;sessionStorage.setItem('idrokPendingAuth',JSON.stringify(pending));document.querySelector('#loginForm').classList.add('hidden');document.querySelector('#registerForm').classList.add('hidden');document.querySelector('#otpForm').classList.remove('hidden');document.querySelector('.auth-panel').classList.add('otp-active');document.querySelector('#otpEmail').textContent=pending.email;const input=document.querySelector('#otpForm input[name="code"]');input.value='';setTimeout(()=>input.focus(),100)}
function finishAuthenticated(result,created=false){authToken=result.token;localStorage.setItem('idrokAuthToken',authToken);const user=applyAuthenticatedUser(result.user);saveState();sessionStorage.setItem('idrokAuthPreviewed','1');sessionStorage.removeItem('idrokPendingAuth');pendingAuth=null;closeAll();showToast(created?'Hisob yaratildi: +50 Impulse!':`Xush kelibsiz, ${user.name}!`);continueAfterAuth()}
async function sendAuthCode({email,name='',create=false}){const service=requireCloudAuth();await service.sendOtp({email,name,create});showOtp({email:String(email).trim().toLowerCase(),name,create,sentAt:Date.now()})}
document.querySelector('#registerForm').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,data=new FormData(form),message=form.querySelector('.form-message'),submit=form.querySelector('[type="submit"]'),label=submit.querySelector('span');message.textContent='';submit.disabled=true;label.textContent='Xat yuborilmoqda…';try{rememberGuestSession();const name=`${data.get('firstName').trim()} ${data.get('lastName').trim()}`.trim();await sendAuthCode({name,email:data.get('email'),create:true})}catch(error){message.textContent=friendlyAuthError(error)}finally{submit.disabled=false;label.textContent='Tasdiqlash xatini yuborish'}});
document.querySelector('#loginForm').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,data=new FormData(form),message=form.querySelector('.form-message'),submit=form.querySelector('[type="submit"]'),label=submit.querySelector('span');message.textContent='';submit.disabled=true;label.textContent='Xat yuborilmoqda…';try{rememberGuestSession();await sendAuthCode({email:data.get('email'),create:false})}catch(error){message.textContent=friendlyAuthError(error)}finally{submit.disabled=false;label.textContent='Xavfsiz kirish xatini yuborish'}});
document.querySelector('#otpForm').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,data=new FormData(form),message=form.querySelector('.form-message'),submit=form.querySelector('[type="submit"]'),label=submit.querySelector('span');message.textContent='';submit.disabled=true;label.textContent='Kod tekshirilmoqda…';try{if(!pendingAuth)throw new Error('Avval emailingizga yangi kod yuboring.');const result=await requireCloudAuth().verifyOtp({email:pendingAuth.email,code:data.get('code')});finishAuthenticated(result,pendingAuth.create)}catch(error){message.textContent=friendlyAuthError(error)}finally{submit.disabled=false;label.textContent='Kodni tasdiqlash'}});
document.querySelector('[data-otp-back]').addEventListener('click',()=>openAuth(pendingAuth?.create?'register':'login'));
document.querySelector('[data-otp-resend]').addEventListener('click',async event=>{const button=event.currentTarget,message=document.querySelector('#otpForm .form-message');button.disabled=true;message.textContent='';try{if(!pendingAuth)throw new Error('Avval emailingizni kiriting.');await requireCloudAuth().sendOtp(pendingAuth);pendingAuth.sentAt=Date.now();sessionStorage.setItem('idrokPendingAuth',JSON.stringify(pendingAuth));message.classList.add('success');message.textContent='Yangi kod yuborildi.'}catch(error){message.classList.remove('success');message.textContent=friendlyAuthError(error)}finally{button.disabled=false}});
document.querySelectorAll('[data-google-auth]').forEach(button=>button.addEventListener('click',async()=>{const original=button.querySelector('span').textContent;button.disabled=true;button.querySelector('span').textContent='Google ochilmoqda…';try{rememberGuestSession();await requireCloudAuth().signInWithGoogle()}catch(error){button.closest('.auth-form').querySelector('.form-message').textContent=friendlyAuthError(error);button.disabled=false;button.querySelector('span').textContent=original}}));
...
async function bootstrapAccount(){
  ...
  if(restored){applyAuthenticatedUser(restored);authModal.classList.remove('open');document.body.style.overflow='';if(requestedTab)history.replaceState(null,'',location.pathname+location.hash);return}
  updateDashboard();renderCourses();renderLeaderboard();
  if(requestedTab==='register'||requestedTab==='login')openAuth(requestedTab==='register'?'register':'login');else openDiscovery();
}
async function configureAuthProviders(){try{const availability=await window.IDROK_AUTH?.providers?.();if(availability&&availability.google===false)document.querySelectorAll('.auth-divider,[data-google-auth]').forEach(element=>{element.hidden=true})}catch{}}
