...
document.querySelector('#registerForm').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,data=new FormData(form),message=form.querySelector('.form-message'),submit=form.querySelector('[type="submit"]'),label=submit.querySelector('span');message.textContent='';submit.disabled=true;label.textContent='Kod yuborilmoqda…';try{rememberGuestSession();const name=`${data.get('firstName').trim()} ${data.get('lastName').trim()}`.trim();await sendAuthCode({name,email:data.get('email'),create:true})}catch(error){message.textContent=friendlyAuthError(error)}finally{submit.disabled=false;label.textContent='Tasdiqlash kodini yuborish'}});
document.querySelector('#loginForm').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,data=new FormData(form),message=form.querySelector('.form-message'),submit=form.querySelector('[type="submit"]'),label=submit.querySelector('span');message.textContent='';submit.disabled=true;label.textContent='Kod yuborilmoqda…';try{rememberGuestSession();await sendAuthCode({email:data.get('email'),create:false})}catch(error){message.textContent=friendlyAuthError(error)}finally{submit.disabled=false;label.textContent='Emailga kod yuborish'}});
