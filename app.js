...
async function renderLeaderboard(){
  ...
  const user=currentUser();leaders.sort((a,b)=>(Number(b[metric])||0)-(Number(a[metric])||0));list.innerHTML=leaders.length?leaders.slice(0,10).map((u,i)=>{const current=u.id===user?.id||u.email===currentEmail,score=Number(u[metric])||0,label=leaderMode==='garden'?`${score} go‘zallik`:leaderMode==='impulse'?`${score} ϟ`:`${score} ball`,safeName=escapeHTML(u.name||'Idrok foydalanuvchisi'),safeInitials=escapeHTML(String(u.name||'ID').slice(0,2).toUpperCase());return `<div class="leader-item ${current?'current':''}"><span class="leader-rank">${String(i+1).padStart(2,'0')}</span><span class="leader-avatar">${safeInitials}</span><span class="leader-name"><b>${safeName}${current?' (Siz)':''}</b><small>${Number(u.gardenPoints)||0} bog‘ ball · ${Number(u.focusMinutes)||0} daqiqa fokus</small></span><span class="leader-score">${label}</span></div>`}).join(''):'<div class="leader-empty"><b>Reyting hali bo‘sh</b><small>Birinchi haqiqiy ishtirokchi bo‘ling.</small></div>';
}
function closeAll(){
